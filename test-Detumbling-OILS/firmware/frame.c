...
uint64_t get_seconds_since_pivot(uint16_t year, uint8_t month, uint8_t date, uint8_t hours, uint8_t minutes, uint8_t seconds)		// ask rishabh about the use of it. I think it is for GOS reading
{
  uint64_t now = 0;
...
void get_seconds_since_equinox(void)  			// verify with Sudin
{
  uint64_t equinox;
  uint64_t seconds_since_pivot;
...
void ecef2eci(vector v_eci, vector v_ecef)
{
  ...
  float phi = W_EARTH_ROT * sidereal_time;						// calculation of relative position from ECEF wrt ECI frame
  
  matrix m_rot = { { cos(phi), sin(phi), 0 },					// since it is Yaw , we apply this rotation matrix
                   { -sin(phi), cos(phi), 0 },
                   { 0, 0, 1 } };
               
  vector_into_matrix(v_eci, m_rot, v_ecef);						// vector into matrix or the reverse , lets see later, see into AE215 notes
}

void eci2ecef(vector v_ecef, vector v_eci)						// same here
{
  float sidereal_time = STPERUT * seconds_since_equinox;
  float phi = W_EARTH_ROT * sidereal_time;
  
  matrix m_rot = { { cos(phi), -sin(phi), 0 },					// inverse of the matrix above
                   { sin(phi), cos(phi), 0 },
                   { 0, 0, 1 } };
...
void ecef2lla(vector v_ecef, vector v_lla)						// ask Sudin
{
...
void ned2ecef(vector v_ned, vector v_lla, vector v_ecef)				// ask Sudin
{
