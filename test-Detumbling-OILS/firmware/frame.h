...
uint64_t get_seconds_since_pivot(uint16_t year, uint8_t month, uint8_t date, uint8_t hours, uint8_t minutes, uint8_t seconds);   // ???what is pivot ????
...
void ecef2lla(vector v_ecef, vector v_lla);			// what is lla - lat-long-alt ok ?
